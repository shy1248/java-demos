 ...
 *
 * checkpoint 是某一时刻整个 flink 任务状态集合的快照，flink 容错机制之一是作业失败可以从 checkpoint 中恢复。 checkpoint 由
 * JM 定时发起，从 source 到 sink 依此完成后结束；flink checkpoint 目前支持以下三种 checkpoint 状态后端：
 *   1. MemeoryStateBackend: 基于内存的状态后端；State 存储在 TM 的内存中，checkpoint 存储在 JM 的内存中，一般为测试时使用；
 *   2. FsStateBackend：基于文件系统的状态后端；State 存储在 TM 的内存中，checkpoint存储在指定的文件系统中（支持本地与HDFS），适合状态数据集较小的作业；
 *   3. RocksDBStateBackend：基于RocksDB 的状态后端；State 存储在 RocksDB（RocksDB是一种基于内存与磁盘的 k-v数据库）中，checkpoint 存储在指定的文件系统中（支持本地与HDFS）
 *
 * flink 容错机制之二是支持作业失败可以自动重启（有不同的重启机制）。 flink 目前支持以下几种自动重启策略：
 *   fixedDelayRestart：固定延时重启策略；需配置最多重启次数以及每次重启之间间隔时间；
 *   failureRateRestart：失败率重启策略；需配置在多长时间之内最多可重启多少次，每次重启之间的时间间隔； 如果没有显式指定重启策略，则采用以下策略：
 *
 * 如果未开启 checkpoint, 重启策略为 None；如果开启了 checkpoint，默认为 fixedDelayRestart 策略，并且最大重启次数为 Integer.MAX_VALUE，每次重启时间间隔为 1 秒钟；
 */
package me.shy.flink.checkpoint;
...
public class CheckpointDemo {
    public static void main(String[] args) {
        ...
        // 解决办法：手动维护 offset
        FlinkKafkaConsumer<String> kafkaConsumer = new FlinkKafkaConsumer<String>("flink-source-demo",
        ...
        SingleOutputStreamOperator<String> wordCounts = env.addSource(kafkaConsumer)
